            ...
            success: function(msg) {
                ...
                if (msg.match(/[^\x01-\x7E]/)) {
                    ...
                    if (jQuery("#jscart_replace_").length > 0) {
                                jQuery("#jscart_replace_").load(EC_WWW_ROOT + "/shop/js/modalcart.aspx?msg=" + msg, function (){ return true;});
                	}
                }else{
        			if (jQuery("#jscart_replace_").length > 0) {
    						...
    						View.DocWriteSwitcher.detach();
                                ...
                                jQuery("#jscart_replace_").load(EC_WWW_ROOT + "/shop/js/modalcart.aspx?goods=" + goodscode_, function (){ return true;});
                	}
                	if (jQuery("div.cart-body div.elm02").length > 0) {
                		jQuery("div.cart-body div.elm02").empty();
                		/* 2016.03.11  kobayayo 不具合対応：#10276 */
                		jQuery("div.cart-body div.elm02").load(EC_WWW_ROOT + '/shop/js/cart.aspx' , 'div.cart-body div.elm02');
                		jQuery("#cart_list").load(EC_WWW_ROOT + '/shop/js/cart.aspx' , '#cart_list');
                	}
                }	
                if (jQuery("select[name='_qtySelect']").length > 0) {
                	 	jQuery("select[name='_qtySelect']").load(EC_WWW_ROOT + "/shop/goods/getstockqtyutil.aspx?goods=" + goodscode_, function (){ return true;});
                }
