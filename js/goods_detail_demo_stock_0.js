...
function GetVariationGoodsCode(varicode,variname1,variname2){ 
	jQuery.ajax({
		async: true,
		type: "POST",
		url: EC_WWW_ROOT + "/res/json/getgoodscodeutil_stock_0.json",
		...
		success: function(code) {
			...
			if (goodscode_ !== '') {
				...
				if (jQuery("select[name='_qtySelect']").length > 0) {
					jQuery("select[name='_qtySelect']").load(EC_WWW_ROOT + "/res/json/getstockqtyutil_stock_0.json?goods=" + goodscode_, function (){
