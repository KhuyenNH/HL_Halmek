...
var goods_detail_load = function(){
	...
	View.setRecommendCarousel = function($parent){
		...
		jQuery('.rel-items-carousel .ifs-direction-nav > li > a',$parent).each(function(){
			...
			var win_width = window.innerWidth;
			var pnBodyList = 0;
			jQuery("#imgSlider .ifs-viewport .slides.large-images li img").each(function(){
				if(pnBodyList < jQuery(this).height()){
					pnBodyList = jQuery(this).height();
				}
			});
...
function GetVariationGoodsCode(varicode,variname1,variname2){ 
	jQuery.ajax({
		async: true,
		type: "POST",
		url: EC_WWW_ROOT + "/shop/goods/getgoodscodeutil.aspx",
		...
		success: function(code) {
			...
			if (goodscode_ !== '') {
				...
				if (jQuery("select[name='_qtySelect']").length > 0) {
					jQuery("select[name='_qtySelect']").load(EC_WWW_ROOT + "/shop/goods/getstockqtyutil.aspx?goods=" + goodscode_, function (){
					...
					if ((jQuery('select[name=_sizeSelect]').val() == '') || (jQuery('select[name=_colorSelect]').val() == '')) {
				 	...
				 	if (deliverydt_ != '' && cartl_ != '') {
				 		...
				 		jQuery('#delivery_comment_sp').text(deliverydt_ + '頃');
				 	}else {
				 		jQuery('#deliveryDate,#deliveryDate_sp').css("display", "none");
				 	}
...
//サイズ一覧テーブルSP表示変換用ファンクション
function sp_size_table_transform(table){
...
jQuery(document).ready( function(){
  if(viewflag != "defult"){
		var table = jQuery("#sizeTableModal .modal-body-iwrap table");
		jQuery("#sizeTableModal .modal-body-iwrap table").replaceWith(sp_size_table_transform(table));
  };
});
