...
jQuery(document).ready(function(){
	...
	jQuery("input[name='use']").bind("change",function(){
		...
		if(boolAction){
			resetNoshiUse(this);
			noshiUseNow = jQuery("input[name=use]").filter(":checked").attr("id");
		}else{
			if(noshiUseNow == ""){
	...
	liquidizeNoshiList("div.thum_item_",120);
...
function judgNextActiveArea(obj,preload){
	if ( jQuery("[name=" + jQuery(obj).attr("name") + "]").filter(":checked").length == 0 ){
		return true;
	}
	...
	if((nowActiveAreaNo == 0) && (jQuery("[name=" + jQuery(obj).attr("name") + "]").filter(":checked").val() == 1)){
		nowActiveAreaNo = 4;
	}
...
function changeNameprintInput(){
	var name = jQuery("input[name='noshi_nameprint']").filter(":checked").val();
	if(name==undefined){
		return;
	}
...
function changePaper(preload){
	...
	jQuery.ajax({
		...
		success: function(data) {
			...
			doLiquidizeNoshiList("div.thum_item_",120);
...
/* 画面幅に合わせて商品表示 */
function liquidizeNoshiList(itemNode,eachMinWidth) {
	if (jQuery(itemNode).length < 1){return;} //要素が見つからない場合は、処理を終了

    jQuery(window).bind('load orientationchange resize', function() {
		doLiquidizeNoshiList(itemNode,eachMinWidth);
    });
}

function doLiquidizeNoshiList(itemNode,eachMinWidth) {
		var padding = parseInt(jQuery(itemNode).css('margin-right'));
		var itemWidth = eachMinWidth + padding * 2;
	
		var parentWidth = jQuery(itemNode).closest("div.thum_box_wrapper_").width();
		...
		jQuery(itemNode).nextAll('br.liquidizeNoshiList').remove();
		jQuery(itemNode + ':nth-child(' + itemCount + 'n)').after("<br class='liquidizeNoshiList' clear='both'>");
}